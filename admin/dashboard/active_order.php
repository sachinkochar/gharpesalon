...
$email=$firms['email'];
   ...
   $sql = 'UPDATE order_payment SET status="C" where `orderid`="'.$id.'"'; 
				$conn->query($sql);   
...
$query = "SELECT DISTINCT(oh.orderid),op.total,usr.name,ba.phone,ba.date,ba.time,op.status FROM order_payment op INNER JOIN order_history oh INNER JOIN users usr INNER JOIN billing_address ba ON op.orderid=oh.orderid AND oh.uid=usr.id AND op.orderid=ba.orderid WHERE status='A'"; 
$result = mysqli_query($conn, $query);
$firms = mysqli_fetch_all($result, MYSQLI_ASSOC);
...
<td><a href="looped_users.php?id=<?php echo $firm['orderid']; ?>">Compelete</a></td></td>
</tr>
<?php endforeach; ?>
</tbody>
